...
describe('run', () => {
  ...
  it('should read the stack outputs from S3', async () => {
    // Arrange
    const getInputMock = jest.spyOn(core, 'getInput').mockImplementation((name: string) => {
      if (name === 'cloud-url') {
  ...
  it('should fail if body is empty', async () => {
    ...
    expect(getObjectMock).toHaveBeenCalled();
    expect(core.setFailed).toHaveBeenCalledWith('Failed to read my-stack from s3://my-bucket/my-prefix/');
  });
